...
export class RippleScrollRenderer {
  ...
  constructor(private _texture: THREE.Texture) {
    ...
    this._createMesh();
    // events
    window.addEventListener("scroll", this._handleScrollMove);
  ...
  private _handleScrollMove = () => {
    const targetImage = document.querySelector(".canvas-image") as Element;
    let offsetTop = targetImage.getBoundingClientRect().top.toString();
    this._mouse.x = 0;
    if (offsetTop.substring(0, offsetTop.length - 1) !== "0") {
      offsetTop.replace(/.$/, "0");
      this._mouse.y = Number(offsetTop);
    }
  };
  ...
  dispose = () => {
    window.removeEventListener("scroll", this._handleScrollMove);
